...
class SwellTests: XCTestCase {
    ...
    func testDefaultDateFormatter() {
        func convertToIntRange(range: Range<String.Index>) -> Range<Int> {
            let start: Int = range.startIndex.distanceTo(range.startIndex)
            let end: Int = range.startIndex.distanceTo(range.endIndex)
            return start..<end
        }
        ...
        XCTAssertEqual(convertToIntRange(quickMatchedRange!), 0..<dateExample.characters.count)
        ...
        XCTAssertEqual(convertToIntRange(flexMatchedRange!), 0..<dateExample.characters.count)
    }
